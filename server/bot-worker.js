...
parentPort.on('message', (msg) => {
  if (msg.type === 'calculate_move') {
    const { stateFen, depth, playerColor, difficulty } = msg.payload;
    const state = engine.parseFen(stateFen);
    
    // Clear transposition table for each new move calculation
    ttClear();
    
    const move = selectBotMove(state, depth, playerColor, difficulty);
    parentPort.postMessage({ type: 'move_result', move });
  }
});
